...
export default {
	...
	data() {
		return {
			...
			keyBoardPopup: false,
			// 搜索热词
			hotWorld: '营养粉', //	搜索热词，如果没有输入即回车，则搜索热词，但是不会加入搜索记录
			// 是否自动聚焦
			focus: true,
			// 语音识别引擎
	...
	computed: {
		colList() {
			// 返回文章和用户列表
			return [
				db.collection(articleDBName).where(this.where).field('thumbnail,title,tags,publish_date,user_id').getTemp(),
	...
	methods: {
		...
		loadList(text = '') {
			...
			if (text) {
				this.where = where + `&& (${new RegExp(text, 'i')}.test(title) || ${new RegExp(text, 'i')}.test(tags))`;
			} else {
				this.where = where;
			}
