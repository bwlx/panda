...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      colList() {
        return [
          db.collection(articleDBName).where(this.where).field('thumbnail,title,tags,publish_date,user_id').getTemp(), // 文章集合
    ...
    methods: {
	  ...
	  initCates() {
		  ...
		  db.collection(categoryDBName).field("_id,name").orderBy("sort asc").get().then((res) => 			{
		    const data = res.result.data
		    if (data) {
		      let dbCates = data.map(item => {
				  return {id: item._id, label: item.name }
			  })	  
			  this.cates.push.apply(this.cates, dbCates)
      ...
      onqueryerror(e) {
        console.error(e);
			...
			cateChange(val){
				...
				let cate = this.cates[val]
				let cate_id = cate['id']
				if (cate_id) {
					this.where = where + `&& "category_id" == "${cate['id']}"`;
				} else {
					this.where = where;
				}
			}
